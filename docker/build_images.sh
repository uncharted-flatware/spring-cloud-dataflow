...
docker buildx build --platform linux/amd64,linux/arm64 -t ghcr.io/unchartedsoftware/spring-cloud-dataflow-server:"$VERSION" -f Dockerfile-dataflow .

# Build skipper-server
docker buildx build --platform linux/amd64,linux/arm64 -t ghcr.io/unchartedsoftware/spring-cloud-skipper-server:"$VERSION" -f Dockerfile-skipper .